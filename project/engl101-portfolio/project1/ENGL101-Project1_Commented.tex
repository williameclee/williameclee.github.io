...
\geometry{left=1in, right=1in, bottom=1in, top=1in, marginparwidth=1.5in, marginparsep=0.1in}
% \geometry{showframe}
...
\definecolor{sandyellow}{RGB}{255, 236, 189}

\usepackage{xkeyval}

\makeatletter
\newcommand{\@comment@fill}{sandyellow}
\newcommand{\@comment@draw}{sandgreen}
\define@key{comment}{draw}[sandgreen]{\def\@comment@draw{#1}}
\define@key{comment}{fill}[sandyellow]{\def\@comment@fill{#1}}
\newif\if@comment@inline
\define@key{comment}{inline}[]{\@comment@inlinetrue}%
\define@key{comment}{noinline}[]{\@comment@inlinefalse}%
...
\newcommand{\comment}[3][]{%
	...
	\if@comment@inline
		\noindent
		\begin{tikzpicture}[remember picture, anchor=base, baseline]
				\setstretch{1}%
				\node[align=left, text width=\textwidth-\the\marginparsep,
					thick, draw=\@comment@draw, fill=\@comment@fill] 
					(Text) at (0,0) {\sffamily\footnotesize #3};
		\end{tikzpicture}
	\else
		\begin{tikzpicture}[remember picture, anchor=base, baseline]
			\coordinate (inText);
			\ifx\relax#2\relax
				\node[anchor=base west, outer sep=0, inner sep=0.2em, minimum height=1.1em] (Text) at (inText) {#2};
			\else
				\node[anchor=base west, fill=\@comment@fill, outer sep=0, inner sep=0.2em, minimum height=1.1em] (Text) at (inText) {#2};
			\fi
		\end{tikzpicture}%
		% Make the margin par
		\marginpar{%
			...
			\begin{tikzpicture}[remember picture]
				\draw node[thick, draw=\@comment@draw, fill=\@comment@fill, text width = \linewidth-\the\marginparsep] (inNote)
					{\sffamily\footnotesize #3};
			\end{tikzpicture}
		}%
		\begin{tikzpicture}[remember picture, overlay]
			\draw[draw = \@comment@draw, thick]
				(Text.south west)
					-| ([xshift=\the\dimexpr-0.5\marginparsep\relax] inNote.west)
					-| (inNote.west);
		\end{tikzpicture}%
	\fi
}
...
\maketitle
\comment{}{Nice title!}
\comment[inline]{}{\textbf{Overall Feedback} \\
William/En-Chi: \\
This was just delightful! Truly a pleasure to read. I especially liked your portrayal and dialogue of Dr. Chen. And of course the descriptions of the rocks and the weirdly generic creek and bridge. \\
Please see the attached PDF for my notes and edits. I changed all of your British/UK style spellings to U.S. style but I do not count off for this (some professor in your future might count off for UK spelling, so it's best to be prepared). \\
But you aced this, William! Congrats on your first ENGL 101 ``A'' essay! More to come, I hope!}
...
the Seaboard Mountain Range loomed to our left, while the majestic Pacific Ocean stretched infinitely to our right, \comment{tantalisingly}{British (U.K.) spelling but I won't take off for it! Just note, U.S. uses a ``z'' that would be an ``s'' in U.K. English.} close.

The bus halted near the No.\,10 Bridge, where a creek met the sea. 
It had no name, for not even the indigenous tribe residing in the mountains had ventured close enough to bother naming it\comment{--}{Type two hyphens for a dash, no space.}it was just a waterway among countless others that poured into the ocean at this lonely stretch of the freeway, and it was \comment{carelessly}{Not sure ``carelessly'' is the word you
want. Perhaps ``generically''?} called the No.\,10 Bridge Creek.
As we hiked upstream along the tranquil stream, the sound of \comment{chatters}{(chatter)} and passing vehicles on the road gradually subsided, yielding the stage to the gentle murmur of running waters. 
It developed into a repertoire accompanied by the rhythmic percussion of my \comment{laboured}{(labored)} breaths as I scaled boulders, occasionally punctuated with trills of our shoes stirring the pebbles that lined the riverbank.

At last, we stopped in a valley carved deep by the creek, kilometres removed from the deafening \comment{civilisation}{(civilization)}. 
...
Swift surveys by us soon identified the predominant rock type comprising the valley walls as conglomerates\comment{--}{(---)}sedimentary rocks resembling inorganic chocolate chip \comment{biscuits}{(cookies?)}. 
In a rushing river, water transported stone fragments, rounding their edges as it flowed. 
As the stream \comment{meandered}{good verb!} \comment{towards}{``toward'' without an ``s'' is preferred in US English. ``Towards'' is more common in the UK.} peace, these pebbles descended to the riverbed, buried within layers of sand and silt. 
...
Upon closer examination of the valley walls, I chanced upon a feature known as the Bouma sequence. This revelation indicated that the conglomerate had not been deposited in a river as we had initially thought, but during a submarine landslide\comment{--}{(---)}a sedimentary condition in which breccia could form. 
...
Might more such fragments\comment{--}{(---)}perhaps roofing tiles or bricks\comment{--}{(---)}lie concealed in this valley? 
The discovery of such an \comment{artefact}{(artifact)} in this remote wilderness defied all my expectations. 
And yet, it seemed to belong here, appearing entirely\comment{...}{Insert a space between each period in an ellipsis: . . .}natural.

No. 
It \textit{was} natural. 
Ultimately, \textit{Homo sapiens} were merely another force of nature. 
In a few tens of thousands of years, this shard would be compressed and cemented into a part of the rock stratum\comment{--}{(---)}a trace fossil, a testament to our passage in the deep geological history.
...
Will they craft a narrative of a mysterious and technologically advanced society that sought to reshape the Earth to suit its desires, or will they dismiss us as merely one of the many \comment{civilisations}{(civilizations)} that ever existed in the stream of time, the same way the Pacific Ocean engulfed a nameless creek? 
...
Our future will be \comment{immortalised}{(immortalized)} in stones. What tales do we want the rocks to impart to those who come after us?
